...
from utils.frozen_ego_minimum.text_encoder import EgoVLP_TextEncoder
...
class EpisodicNLQProcessor:
        ...
        for vid, data_item in tqdm(data.items(), total=len(data), desc=f"process episodic nlq {scope}"):
            ...
            for timestamp, exact_time, sentence, ann_uid, query_idx in zipper:
                ...
                if self._predictor == "EgoVLP":
                    words = self.text_encoder.encode([sentence.strip().lower()])
                else:
                    words = sentence
                record = {
    ...
    def convert(self, data_dir, configs):
        predictor = configs.predictor
        self._predictor = predictor
        if self._predictor == "EgoVLP":
            self.text_encoder = EgoVLP_TextEncoder(configs.fv)
...
def gen_or_load_dataset(configs):
    ...
    if configs.suffix is None:
        save_path = os.path.join(
            configs.save_dir,
            "_".join(
                [configs.task, configs.fv, str(configs.max_pos_len)]
            )
            + ".pkl",
        )
    else:
